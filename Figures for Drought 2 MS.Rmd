...
l_leg <- read.csv("Processed Data/legacy_modeling_data.csv") %>% 
  left_join(sitechar) %>% 
  group_by(Species) %>%
...
site_area <- read.csv("Processed Data/site_area.csv")
dia_data <- read.csv("Processed Data/field_data.csv") %>% 
  filter(Site2!="ic") %>% 
  left_join(site_area)

trees_area <- group_by(dia_data, Region, hilo, Site, Neighborhood, Species) %>% 
  summarise(density = length(unique(tree.uniqueID)),
            BA = sum(totBA, na.rm=T),
            area_m=mean(area),
            area=area_m*0.0001,
            tph = density/(area),
            BA_ph=BA/(area))
...
map2 <- ggplot() +
  ...
  labs(color="Latitude") +
  theme(panel.background = element_rect(fill = "gray"), axis.title=element_blank(),
        axis.text=element_blank(), axis.ticks = element_blank()) +
  theme(legend.position = 'none') 

bivar <- ggplot(site_clim, aes(avg_temp, avg_ppt, color=Region)) +
  geom_point(size=2) +
...
dummy <- ggplot(avg_clim, aes(Region, avg_ppt, color=Region)) +
...
p2 <- ggdraw() + draw_image("images/drought measurements new.svg") 
p2

mapgrob <- as_grob(map2)

bivar2 <- bivar + annotation_custom(grob=mapgrob, ymin = 890, ymax = 1122, xmin = 4.4, xmax = 7.4)

plot_grid(
  plot_grid(
    plot_grid(p2, map2, bivar, ncol=1, labels = c('a)', 'b)', 'c)'), rel_heights = c(0.6, 1, 1), align = "v", scale=c(1.08, 0.9, 0.9)),
    drought, ncol=2, rel_widths = c(.30, .7), labels=c('', 'd)')),
  legend, ncol=1, rel_heights = c(1, 0.05))


plot_grid(plot_grid(p2, drought, ncol=1, rel_heights=c(0.3, 0.7), c("a)", "c)")),
          plot_grid(map, bivar, ncol=1, rel_heights=c(1, 1)), nrow=1, rel_widths=c(0.6, 0.4), labels=c("b)", "d)"))

ggplot(dia_data, aes(interaction(hilo, Region))) +
...
trees_area2 <- group_by(trees_area, Region, hilo, Site, Species) %>% 
  summarise_all(mean)

ggplot(trees_area, aes(x=interaction(Neighborhood))) +
  geom_bar(aes(y=tph, fill=Species), stat="identity", position="dodge") +
...
p1 <- ggplot(trees_area2, aes(x=interaction(hilo, Region))) +
  geom_bar(aes(y=tph, fill=Species), stat="identity") +
  scale_fill_viridis(discrete=T) +
  ylab("Density (trees per hectare)") +
  theme_test() +
  theme(legend.position = 'none') +
...
p2 <- ggplot(trees_area2, aes(x=interaction(hilo, Region))) +
  geom_bar(aes(y=BA_ph, fill=Species), stat="identity") +
  scale_fill_viridis(discrete=T) +
  ylab("Basal area (m^2 per hectare)") +
  theme_test() +
  theme(legend.position = 'none') +
  theme(axis.title.x=element_blank(),
        axis.text.x = element_text(angle = 45, hjust=1))

dummy <- ggplot(trees_area2, aes(x=interaction(hilo, Region))) +
  geom_bar(aes(y=BA_ph, fill=Species), stat="identity") +
  scale_fill_viridis(discrete=T) +
  theme_test() +
...
plot_grid(p1, p2, legend, nrow=1, labels=c("a)", "b)"), rel_widths=c(1,1,0.2))
...
resp3b <- readRDS("Brms models/final rr models/resp3b.rds")
recov4b <-readRDS("Brms models/final rr models/recov4b.rds")
```
#### Growth Reduction
...
w2 <- cbind(waic_diff = w[, 1] * -2,
      se        = w[, 2] *  2) %>% round(digits=2)
kable(w2, "html", caption="Table SX. Growth reduction WAIC table") 

`Species var` <- add_criterion(resp3, "waic")
`Global var` <- add_criterion(resp3b, "waic")

wb <- loo_compare(`Species var`, `Global var`, criterion = "waic") 

w2b <- cbind(waic_diff = wb[, 1] * -2,
      se        = wb[, 2] *  2) %>% round(digits=2)
kable(w2b, "html", caption="Table SX. Growth reduction WAIC table variance term") 
```
...
`Species var` <- add_criterion(recov4, "waic")
`Global var` <- add_criterion(recov4b, "waic")

wb <- loo_compare(`Species var`, `Global var`, criterion = "waic")

w2b <- cbind(waic_diff = wb[, 1] * -2,
      se        = wb[, 2] *  2) %>% round(digits=2)
kable(w2b, "html", caption="Table SX. Recovery WAIC table, variance term") 
```
...
plot_grid(plot_grid(p1, p2, ncol=1, labels = c("a)", "b)")), legend, ncol=2, rel_widths = c(1, 0.2))
```

#### Main Figure
```{r q1_mainfig, out.width="70%"}
p1 <- l_leg %>%
  group_by(Species) %>%
...
plot_grid(p1, p2, legend, ncol=3, rel_widths = c(1, 1, 0.3), labels = c("a)", "b)"))
...
icc(resp8_ac, re.form=~(1|Region))
icc(resp8_pj, re.form=~(1|Region))
icc(resp8_pl, re.form=~(1|Region))
```
...
icc(recov8_ac, re.form=~(1|Region))
icc(recov8_pj, re.form=~(1|Region))
icc(recov8_pl, re.form=~(1|Region))
...
resp_param <- bind_rows(b_ac, b_pj, b_pl) %>% 
  mutate(param=factor(param, levels=c("Intercept", "spei12")),
         param2=ifelse(param=="Intercept", "Intercept", "Slope"))
...
dat_text <- data.frame(
  label = c("A. concolor", "P. jeffreyi", "P. lambertina"),
  Species   = c("AC", 'PJ', "PL")
)

p1 <- ggplot(resp_param, aes(y = param2, x = region_mean)) +
  geom_pointintervalh(aes(xmin = .lower, xmax = .upper, color=Region), position = position_dodgev(-0.4)) + 
  geom_vline(xintercept=0, linetype="dashed") +
  ...
  theme(plot.margin = unit(c(0.1,0.1,0.1,0.1), "cm"))  + 
  geom_text(size = 3.5, fontface = 3, data = dat_text,
    mapping = aes(x = -Inf, y = Inf, label = label),
    hjust = -.05,vjust = 1.5)


p2 <- ggplot(recov_param, aes(y = param2, x = region_mean)) +
  geom_pointintervalh(aes(xmin = .lower, xmax = .upper, color=Region), position = position_dodgev(-0.4)) + 
  geom_vline(xintercept=0, linetype="dashed") +
  facet_grid(~Species, labeller=labeller(.cols=def)) +
  theme_test() +
  scale_color_viridis(discrete=T, option="plasma") +
  xlab("Parameter estimate") + theme(legend.position = 'none') +
  ylab("Recovery") +
  theme(strip.background = element_blank(), strip.text = element_text(hjust=0, face="bold", size=rel(1.0))) +
  theme(plot.margin = unit(c(0.1,0.1,0.1,0.1), "cm")) + 
  geom_text(size = 3.5, fontface = 3, data = dat_text,
...
plot_grid(plot_grid(p1, p2, cols=1, align='vh'), legend, ncol=2, rel_widths = c(1, 0.2))

sp_lab <- c(AC="A. concolor", PJ="P. jeffreyi", PL="P. lambertiana")

ggplot(resp_cont, aes(x=region_mean, y=region, color=signif)) +
  geom_pointintervalh() + 
  facet_grid(Species~param2, labeller=labeller(.rows=sp_lab)) +
  geom_vline(xintercept = 0, linetype="dashed") +
  theme_test() +
...
p1 <- l_leg %>%
  ...
  ggplot(aes(x = spei12, y = .value)) +
  geom_point(data=l_leg, aes(spei12, drought_fx, color = Region), alpha=0.3) +
  stat_lineribbon(aes(y = .value, color = Region), .width = c(.95), alpha = 0.4) +
  stat_lineribbon(aes(y = .value, color = Region), .width = .01) +
  scale_fill_brewer(palette = "Greys") +
...
p2 <- l_leg %>%
  ...
  ggplot(aes(x = drought_fx, y = .value)) +
  geom_point(data=l_leg, aes(drought_fx, recovery, color = Region), alpha=0.3) +
  stat_lineribbon(aes(y = .value, color = Region), .width = c(.95), alpha = 0.4) +
  stat_lineribbon(aes(y = .value, color = Region), .width = .01) +
  scale_fill_brewer(palette = "Greys") +
  scale_color_viridis(discrete=TRUE, option = 'plasma') +
...
dummy <- ggplot(l_leg, aes(x = drought_fx, y = recovery, color = Region)) +
  geom_line() + scale_color_viridis(discrete=T, option="plasma") +
  theme_test()

legend <- cowplot::get_legend(dummy)

plot_grid(plot_grid(p1, p2, cols=1, align='vh'), legend, ncol=2, rel_widths = c(1, 0.15))
...
### Q3
```{r q3_load}
resp_full_ac <- readRDS("Brms models/final rr models/resp_full_ac_scaled.rds")
resp_full_pj <- readRDS("Brms models/final rr models/resp_full_pj_scaled.rds")
resp_full_pl <- readRDS("Brms models/final rr models/resp_full_pl_scaled.rds")

recov_full_ac <- readRDS("Brms models/final rr models/recov_full_ac_scaled.rds")
recov_full_pj <- readRDS("Brms models/final rr models/recov_full_pj_scaled.rds")
recov_full_pl <- readRDS("Brms models/final rr models/recov_full_pl_scaled.rds")
```
  ...
  temp <- mod %>% 
    ...
    mutate(region="all",
           param=ifelse(.variable=="b_DBH", "DBH",
                 ...
                 ifelse(.variable==vars[[4]], "DBH interaction", "Comp interaction"))))
...
resp_all <- bind_rows(p_ac, p_pj, p_pl) %>% 
  mutate(region=factor(region, levels=c("all", "San.Jac", "SENF", "SEKI", "Mammoth")),
         param=factor(param, levels=c("Intercept", "Slope", "DBH", "Comp", "DBH interaction", "Comp interaction")),
...
get_scaled_data <- function(data, spp) {
  data %>% 
    mutate(spei12_raw=spei12,
           drought_fx_raw=drought_fx) %>% 
    mutate_at(vars(spei12, drought_fx, DBH, comp), scale) %>% 
    filter(Species==spp) %>% 
    mutate(Region=droplevels(Region))
}

get_pred_data <- function(data, model) {
  if(str_detect(deparse(substitute(model)), "resp")) {
    pred <- data.frame(spei12 = seq_range(data$spei12, n = 10),
              spei12_raw = seq_range(data$spei12_raw, n = 10))
    
    data %>%
    group_by(Region) %>% 
    data_grid(Y, pred,
              DBH = quantile(data$DBH, probs=c(.025, .5, .975), na.rm=T),
              comp = quantile(data$comp, probs=c(.025, .5, .975), na.rm=T),
              tree.uniqueID=1) %>%
    ...
    mutate(DBH=as.factor(round(DBH*data$sd_dbh+data$mean_dbh, digits=2)),
       ...
       pred <- data.frame(drought_fx = seq_range(data$drought_fx, n = 10),
              drought_fx_raw = seq_range(data$drought_fx_raw, n = 10))
    data %>%
    group_by(Region) %>% 
    data_grid(Y, pred,
              DBH = quantile(data$DBH, probs=c(.025, .5, .975), na.rm=T),
...
```{r q3_fig}  
# 
# bayes_R2(resp_full_ac)[1]-bayes_R2(resp8_ac)[1]
# bayes_R2(resp_full_pj)[1]-bayes_R2(resp8_pj)[1]
# bayes_R2(resp_full_pl)[1]-bayes_R2(resp8_pl)[1]
# bayes_R2(recov_full_ac)[1]-bayes_R2(recov8_ac)[1]
# bayes_R2(recov_full_pj)[1]-bayes_R2(recov8_pj)[1]
# bayes_R2(recov_full_pl)[1]-bayes_R2(recov8_pl)[1]

sp_lab <- c(AC="A. concolor", PJ="P. jeffreyi", PL="P. lambertiana")
p1 <- ggplot(filter(resp_all, global=="global"), aes(y = param, x = .value, xmin = .lower, xmax = .upper, color=region)) +
  geom_vline(xintercept = 0, linetype="dashed") +
  geom_pointintervalh(position = position_dodgev(-0.5)) +
  facet_wrap(~Species, ncol=3, scales="free_x",
             labeller=labeller(.cols=sp_lab)) +
  # facet_grid(~Species) +
  theme_test() +
  # theme(legend.position = 'none') +
  xlab("Parameter estimate") + 
...
p2 <- ggplot(filter(resp_all, global!="global"), aes(y = param, x = .value, xmin = .lower, xmax = .upper, color=region)) +
  geom_vline(xintercept = 0, linetype="dashed") +
  geom_pointintervalh(position = position_dodgev(-0.5)) +
  facet_wrap(~Species, ncol=3, scales="free_x",
             labeller=labeller(.cols=sp_lab)) +
  ...
  # theme(legend.position = 'none') +
  xlab("Parameter estimate") + 
  scale_color_manual(values=c("#0D0887FF", "#9C179EFF", "#ED7953FF", "#F0F921FF"))+
  theme(strip.background = element_blank(), strip.text = element_text(face='italic')) +
  theme(axis.title=element_blank()) +
  theme(plot.margin = unit(c(0.1,0.1,0.1,0.1), "cm")) +
  theme(legend.position = 'none')
...
p1 <- ggplot(filter(recov_all, global=="global"), aes(y = param, x = .value, xmin = .lower, xmax = .upper, color=region)) +
  geom_vline(xintercept = 0, linetype="dashed") +
  geom_pointintervalh(position = position_dodgev(-0.5)) +
  facet_wrap(~Species, ncol=3, scales="free_x",
             labeller=labeller(.cols=sp_lab)) +
...
p2 <- ggplot(filter(recov_all, global!="global"), aes(y = param, x = .value, xmin = .lower, xmax = .upper, color=region)) +
  geom_vline(xintercept = 0, linetype="dashed") +
  geom_pointintervalh(position = position_dodgev(-0.5)) +
  facet_wrap(~Species, ncol=3, scales="free_x",
             labeller=labeller(.cols=sp_lab)) +
...
p1 <- ggplot(filter(dbh_param, response=="resp"), aes(y = param, x = .value)) +
  geom_vline(xintercept = 0, linetype="dashed") +
  geom_pointintervalh(aes(xmin = .lower, xmax = .upper, color=signif95)) +
  scale_color_grey(start=0.8, end=0) +
...
p2 <- ggplot(filter(dbh_param, response=="recov"), aes(y = param, x = .value)) +
  geom_vline(xintercept = 0, linetype="dashed") +
  geom_pointintervalh(aes(xmin = .lower, xmax = .upper, color=signif95)) +
  scale_color_grey(start=0.8, end=0) +
...
```{r pred_dat}
ac_dat <- get_scaled_data(l_leg, "AC")
pj_dat <- get_scaled_data(l_leg, "PJ")
pl_dat <- get_scaled_data(l_leg, "PL")

ac_pred <- get_pred_data(ac_dat, resp_full_ac) 
pj_pred <- get_pred_data(pj_dat, resp_full_pj)
pl_pred <- get_pred_data(pl_dat, resp_full_pl)

senf_resp <- bind_rows(mutate(ac_pred, Species="ac"),
                       mutate(pj_pred, Species="pj"),
                       mutate(pl_pred, Species="pl")) %>% 
  filter(Region=="SENF") %>% 
  mutate(response="growth reduction",
         DBH=as.numeric(DBH),
         comp=as.numeric(comp),
         xvar=spei12) %>% 
  group_by(Species) %>% 
  mutate(dbh2=factor(DBH, labels=c("small", "med", "large")), 
         comp2=factor(comp, labels=c("low", "med", "high")))


ac_pred2 <- get_pred_data(ac_dat, recov_full_ac)
pj_pred2 <- get_pred_data(pj_dat, recov_full_pj)
pl_pred2 <- get_pred_data(pl_dat, recov_full_pl)

senf_recov <- bind_rows(mutate(ac_pred2, Species="ac"),
                       mutate(pj_pred2, Species="pj"),
                       mutate(pl_pred2, Species="pl")) %>% 
  filter(Region=="SENF") %>% 
...
p1 <-ggplot(senf_resp, aes(x = spei12_raw, y = .value)) +
  # geom_point(data=filter(l_leg, Species=="PL"), aes(drought_fx, recovery),alpha=0.3) +
  stat_lineribbon(aes(y = .value, color=dbh2), .width = c(.95), alpha = 0.4) +
  stat_lineribbon(aes(y = .value, color=dbh2), .width = .01) +
  scale_fill_brewer(palette = "Greys") +
  ...
  ylab("Growth reduction") +
  guides(fill=FALSE) +
  # ggtitle("Sequoia National Forest") +
  theme(strip.background = element_blank(), strip.text = element_text(hjust=0, face="bold", size=rel(1.0))) +
  labs(color="DBH") +
  theme(plot.margin = unit(c(0.1,0.1,0.1,0.1), "cm")) +
  theme(legend.position = 'none') +
  scale_x_reverse(labels=function(x) {round(x*unique(ac_dat$sd_spei12)+(unique(ac_dat$mean_spei12)), digits=1)}) + 
  geom_text(size = 3.5, fontface = 3, data = dat_text,
    mapping = aes(x = Inf, y = Inf, label = label),
    hjust = -.05,vjust = 1.5)

p2 <-ggplot(senf_recov, aes(x = drought_fx_raw, y = .value)) +
  # geom_point(data=filter(l_leg, Species=="PL"), aes(drought_fx, recovery),alpha=0.3) +
  stat_lineribbon(aes(y = .value, color=dbh2), .width = c(.95), alpha = 0.4) +
  stat_lineribbon(aes(y = .value, color=dbh2), .width = .01) +
  scale_fill_brewer(palette = "Greys") +
...
dummy <- ggplot(senf_recov, aes(x = drought_fx_raw, y = .value, color=dbh2)) +
  # geom_bar(stat = "summary", fun.y = "mean") +
  geom_line(size=1.5)+
...
plot_grid(plot_grid(p1, p2, ncol=1, align='vh', axis='trbl'), legend, ncol=2, rel_widths=c(1, 0.15))

p1 <-ggplot(senf_resp, aes(x = spei12_raw, y = .value)) +
  # geom_point(data=filter(l_leg, Species=="PL"), aes(drought_fx, recovery),alpha=0.3) +
  stat_lineribbon(aes(y = .value, color=comp2), .width = c(.95), alpha = 0.4) +
  stat_lineribbon(aes(y = .value, color=comp2), .width = .01) +
  scale_fill_brewer(palette = "Greys") +
  scale_color_viridis(discrete=TRUE) +
  facet_grid(~Species, labeller = labeller(.cols=abc)) +
  geom_hline(yintercept = 0, linetype="dashed") +
...
dummy <- ggplot(senf_recov, aes(x = drought_fx_raw, y = .value, color=comp2)) +
  # geom_bar(stat = "summary", fun.y = "mean") +
  geom_line(size=1.5)+
  scale_color_viridis(discrete=T) +
  theme_test() +
...
ggplot(ac_pred, aes(x = spei12, y = .value, color = Region)) +
  geom_point(data=ac_dat, aes(spei12, drought_fx, color = Region),alpha=0.3) +
  stat_lineribbon(aes(y = .value), .width = c(.95), alpha = 0.4) +
  stat_lineribbon(aes(y = .value), .width = .01) +
  scale_fill_brewer(palette = "Greys") +
  scale_color_viridis(discrete=TRUE, option = 'plasma') +
...
ggplot(ac_pred, aes(x = spei12, y = .value)) +
...
ggplot(pj_pred, aes(x = spei12, y = .value, color = Region)) +
  geom_point(data=pj_dat, aes(spei12, drought_fx, color = Region),alpha=0.3) +
  stat_lineribbon(aes(y = .value), .width = c(.95), alpha = 0.4) +
  stat_lineribbon(aes(y = .value), .width = .01) +
  scale_fill_brewer(palette = "Greys") +
  scale_color_viridis(discrete=TRUE, option = 'plasma') +
  scale_x_reverse(labels=function(x) {round(x*unique(pj_dat$sd_spei12)+(unique(pj_dat$mean_spei12)), digits=1)}) +
  # ylim(-.4, .4) +
  theme_test() +
  ylab("Growth reduction") +
  xlab("Drought intensity") +
...
ggplot(pj_pred, aes(x = spei12, y = .value)) +
...
ggplot(pl_pred, aes(x = spei12, y = .value, color = Region)) +
  geom_point(data=pl_dat, aes(spei12, drought_fx, color = Region),alpha=0.3) +
  stat_lineribbon(aes(y = .value), .width = c(.95), alpha = 0.4) +
  stat_lineribbon(aes(y = .value), .width = .01) +
  scale_fill_brewer(palette = "Greys") +
  scale_color_manual(values=c("#0D0887FF", "#9C179EFF", "#ED7953FF"))+
  scale_x_reverse(labels=function(x) {round(x*unique(pl_dat$sd_spei12)+(unique(pl_dat$mean_spei12)), digits=1)}) +
  # ylim(-.4, .4) +
  theme_test() +
  ylab("Growth reduction") +
  xlab("Drought intensity") +
...
ggplot(pl_pred, aes(x = spei12, y = .value)) +
...
ggplot(ac_pred2, aes(x = drought_fx, y = .value, color = Region)) +
  geom_point(data=ac_dat, aes(drought_fx, recovery, color = Region),alpha=0.3) +
  stat_lineribbon(aes(y = .value), .width = c(.95), alpha = 0.4) +
  stat_lineribbon(aes(y = .value), .width = .01) +
  scale_fill_brewer(palette = "Greys") +
  scale_color_viridis(discrete=TRUE, option = 'plasma') +
  ...
  guides(fill=FALSE) +
  ggtitle("AC Regional effects") +
  scale_x_continuous(labels=function(x) {round(x*unique(ac_dat$sd_dfx)+(unique(ac_dat$mean_dfx)), digits=1)})

ggplot(ac_pred2, aes(x = drought_fx, y = .value)) +
...
```{r recov_full pj}
ggplot(pj_pred2, aes(x = drought_fx, y = .value, color = Region)) +
  geom_point(data=pj_dat, aes(drought_fx, recovery, color = Region),alpha=0.3) +
  stat_lineribbon(aes(y = .value), .width = c(.95), alpha = 0.4) +
  stat_lineribbon(aes(y = .value), .width = .01) +
  scale_fill_brewer(palette = "Greys") +
  scale_color_viridis(discrete=TRUE, option = 'plasma') +
  ...
  guides(fill=FALSE) +
  ggtitle("PJ Regional effects") +
  scale_x_continuous(labels=function(x) {round(x*unique(pj_dat$sd_dfx)+(unique(pj_dat$mean_dfx)), digits=1)})

ggplot(pj_pred2, aes(x = drought_fx, y = .value)) +
...
ggplot(pl_pred2, aes(x = drought_fx, y = .value, color = Region)) +
  geom_point(data=pl_dat, aes(drought_fx, recovery, color = Region),alpha=0.3) +
  stat_lineribbon(aes(y = .value), .width = c(.95), alpha = 0.4) +
  stat_lineribbon(aes(y = .value), .width = .01) +
  ...
  guides(fill=FALSE) +
  ggtitle("PL Regional effects") +
  scale_x_continuous(labels=function(x) {round(x*unique(pl_dat$sd_dfx)+(unique(pl_dat$mean_dfx)), digits=1)})

ggplot(pl_pred2, aes(x = drought_fx, y = .value)) +
  # geom_point(data=filter(l_leg, Species=="PL"), aes(drought_fx, recovery),alpha=0.3) +
  stat_lineribbon(aes(y = .value, color=comp), .width = c(.95), alpha = 0.4) +
  stat_lineribbon(aes(y = .value, color=comp), .width = .01) +
  scale_fill_brewer(palette = "Greys") +
