...
test <- newdat %>%
  group_by(Region) %>% 
  data_grid(Y, spei12 = seq_range(spei12, n = 10),
            DBH = c(min(newdat$DBH), 0, max(newdat$DBH)),
            comp = c(min(newdat$comp), 0, max(newdat$comp)),
            tree.uniqueID=1) %>%
...
ggplot(test, aes(x = spei12, y = .value, color = Region)) +
  geom_point(data=newdat, aes(spei12, drought_fx, color = Region),alpha=0.3) +
  stat_lineribbon(aes(y = .value), .width = c(.95), alpha = 0.4) +
  stat_lineribbon(aes(y = .value), .width = .01) +
  scale_fill_brewer(palette = "Greys") +
  scale_color_viridis(discrete=TRUE, option = 'plasma') +
  ...
  scale_x_continuous(labels=function(x) {x*newdat$sd_spei12+(newdat$mean_spei12)}) +
  scale_x_reverse()
...
ggplot(test, aes(x = spei12, y = .value)) +
  ...
  scale_x_continuous(labels=function(x) {x*newdat$sd_spei12+(newdat$mean_spei12)}) +
  scale_x_reverse() +
  guides(color=guide_legend(labels = element_text(c("A", "B", "C"))))
