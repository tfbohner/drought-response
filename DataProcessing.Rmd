...
dia_data2 <- dia_data %>% 
  ...
  mutate(Species=tolower(paste(str_sub(Species, 1,1), str_sub(Species, 3,3), sep="")),
         ...
         totDBH=rowSums(dplyr::select(., "DBH", "DBHstem2", "DBHstem3"), na.rm=T),
         totDBH=ifelse(totDBH==0, NA, totDBH),
         totDBH=ifelse(!is.na(DBHT3), DBHT3, 
                       ifelse(!is.na(DBHT2), DBHT2, totDBH))) %>% 
  
  dplyr::select(c(tree.uniqueID, totDBH))
# 
...
dfx_summary <- dfx_data %>% 
  group_by(Region, Site, hilo, Species, Neighborhood, tree.uniqueID, droughtyear, hegyi, simple_comp, BA, DBH) %>% 
...
for(i in unique(dfx_data$tree.uniqueID)){ # unique(dfx_data$tree.uniqueID)
  ...
  for(j in unique(dfx_data$Year[which(dfx_data$tree.uniqueID==i)])) {
    ...
    pred_grow <- fitted(mod, newdata=newdata, summary=FALSE, nsamples = 1000)
...
legacy <- big_df %>%
  ...
  left_join(dplyr::select(dfx_data, c(Year, Site, Species, Neighborhood, Region, hilo, tree.uniqueID, hegyi, simple_comp, BA, DBH, reduction,
